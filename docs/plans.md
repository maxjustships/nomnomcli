...
## Final Semantic Planning Review Source
- Task: Fix P1 exact-product identity leakage and P2 symlink metadata mutation in read-only semantic planning.
- Canonical input: Final semantic planning review plus the user's no-write, regression, validation, local-commit, and no-push requirements.
- Repo context: semantic raw resolution, local cache identity, Linux SQLite snapshot traversal, and strict JSON CLI tests.
- Last updated: 2026-07-21

## Final Review Assumptions
- A raw `exact_product` plan is safe only when the original query has exact alias/name/lookup identity, matching barcode/SKU evidence, or matching returned brand identity.
- Linux descriptor-relative opens are the source of truth: parent components use no-atime `O_PATH|O_DIRECTORY|O_NOFOLLOW`, and SQLite files use `O_NOATIME|O_NOFOLLOW`; path resolution and pre-open metadata calls are forbidden.
- Missing sidecars remain valid, but any final or parent-component symlink in the database/sidecar traversal fails closed before source bytes are copied.

## Final Review Milestone Order
| ID | Title | Depends on | Status |
| --- | --- | --- | --- |
| M31 | Gate raw exact plans on proven identity | M30 | [x] |
| M32 | Replace snapshot path traversal with no-follow descriptors | M31 | [x] |
| M33 | Run release gates, audit, and commit | M32 | [x] |

## M31. Gate raw exact plans on proven identity `[x]`
### Goal
- A partial ranked cache hit with conflicting identity is never emitted as `exact_product`; valid exact pin, alias, barcode/SKU, and matching-brand plans remain unchanged.

### Tasks
- [x] Add a no-write conflicting pinned-brand regression and positive exact-identity coverage.
- [x] Require proven original-query identity before returning any raw `exact_product` plan.
- [x] Continue semantic candidates when safe, otherwise return a structured exact/no-safe-resolution refusal.

### Definition of Done
- No raw exact plan can be produced solely from partial token overlap.
- Refusals preserve strict JSON and leave the database unchanged.

### Validation
```sh
rtk pytest -q tests/test_semantic.py -k 'identity or alias or barcode or pin'
```

### Known Risks
- The gate must distinguish an exact local lookup pin from a ranked partial match without weakening provider brand/barcode identity.

### Stop-and-Fix Rule
- Any exact alias/name/lookup, barcode/SKU, or matching-brand regression blocks M32.

## M32. Replace snapshot path traversal with no-follow descriptors `[x]`
### Goal
- Resolve snapshots never follow or touch metadata on user-supplied symlinks and remain race-safe while preserving OFD, no-atime, WAL, rollback, and MEMORY/OFF guarantees.

### Tasks
- [x] Add stale-atime final, parent, and sidecar symlink strict JSON/no-write regressions plus ordinary-file success coverage.
- [x] Open every parent descriptor-relative with `O_PATH|O_NOFOLLOW` and every SQLite source/sidecar with `O_NOFOLLOW|O_NOATIME`.
- [x] Fingerprint only validated descriptors and fail closed on topology or inode changes before accepting bytes.

### Definition of Done
- Final and parent symlinks retain atime, mtime, target/content, and directory state on refusal.
- Ordinary database, WAL/rollback, and active-writer behavior remains covered.

### Validation
```sh
rtk pytest -q tests/test_semantic.py -k 'snapshot or symlink or noatime or pending_wal or rollback or journal_less'
```

### Known Risks
- Directory lookup races and sidecar appearance/disappearance must never reopen by an unvalidated absolute path.

### Stop-and-Fix Rule
- Any source metadata mutation, followed symlink, unlocked copy, or weakened writer refusal blocks M33.

## M33. Run release gates, audit, and commit `[x]`
### Goal
- Targeted/full tests, Ruff, diff checks, historical-doc absence, and a scoped conventional local commit all pass without a push or PR.
...
### Stop-and-Fix Rule
- Do not commit until every gate passes and the diff contains no historical documentation churn.
