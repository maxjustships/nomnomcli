...
4. Build a version-2 plan with `accuracy_profile`, raw `input`, optional positive `grams`, and one strict selection/direct ref/pending state. Unbranded selection uses `semantic_equivalent`.
   Every selection requires `semantic_attestation` with only version 1, matching relation, exact discovery query as `raw_identity`, exact candidate `semantic_identity` as `selected_identity`, `same_food_type:true`, concise rationale, and confidence 0..1. Assert it only after externally judging the identities. nomnom does not trust that assertion alone: it revalidates the candidate and requires deterministic literal-anchor plus provider type/category evidence, failing closed when compatibility is not established.
...
5. Run `nomnom agent intake --plan 'JSON' --json`; it re-runs discovery for every selection, verifies branded receipts and deterministic semantic evidence, re-fetches facts, calculates, and writes atomically. Show profile, source, relation, assumption, search status, totals, completeness, and pending IDs.
