...
This command is dry-run only. Require `would_write=false`; show the separate `original` and `retrieval_query`, candidate index/relation/assumption, source identity/type, confidence, alternatives, and `resolution_mode`. The original is tried first. Candidate results may be only `generic_proxy`, never `exact_product`; barcode, SKU, and explicit-brand input still requires exact capture even if `brand_intent` is false. With no static brand/synonym corpus, a same-language candidate that shares tokens but drops an original token conservatively requires exact capture; a provider-returned candidate whose brand tokens match the full original, including brand-only input, is runtime exact-intent evidence. This may refuse legitimate simplification, while translated brand intent must still be declared explicitly. On supported 64-bit Linux runtimes, OS-only open-file-description locks freeze SQLite's main lock range and WAL SHM slots while the main, rollback-journal, WAL, and SHM set is copied and fingerprinted. Active writers, including MEMORY/OFF transactions with dirty spilled pages, return structured `database_snapshot_busy`; unavailable compatible locking returns `database_snapshot_lock_unavailable`; repeated sidecar churn returns `database_snapshot_unstable`. Every refusal has `would_write:false`. SQLite opens only the private copy, never the source. Unsafe candidates return a structured refusal. Do not apply the returned plan to a log, create an alias, change policy/config, or manipulate SQLite. There is no `log --resolution-intents` workflow in this phase.
