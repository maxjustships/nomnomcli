...
This command is dry-run only. Require `would_write=false`; show the separate `original` and `retrieval_query`, candidate index/relation/assumption, source identity/type, confidence, alternatives, and `resolution_mode`. The original is tried first. Candidate results may be only `generic_proxy`, never `exact_product`; barcode, SKU, and explicit-brand input still requires exact capture even if `brand_intent` is false. With no static brand/synonym corpus, a same-language candidate that shares tokens but drops an original token conservatively requires exact capture; this may refuse legitimate simplification, while translated brand intent must still be declared explicitly. Existing state is cloned read-only and only the isolated in-memory snapshot is migrated. Unsafe candidates return a structured refusal. Do not apply the returned plan to a log, create an alias, change policy/config, or manipulate SQLite. There is no `log --resolution-intents` workflow in this phase.
